...
# Author: Dhyan M Setty
# Description: Real-time face recognition attendance system 
#              using OpenCV (LBPH), Tkinter GUI, and CSV storage.
# ============================================================
...
# -------------------- Auto-create required project folders --------------------
required_dirs = ['Attendance', 'StudentDetails', 'TrainingImage', 'TrainingImageLabel']
...
# Verify Haar cascade file exists for face detection
...
# Save or update training password
def save_pass():
    assure_path_exists("TrainingImageLabel/")
    exists1 = os.path.join("TrainingImageLabel", "psd.txt")
    if exists1:
...
# Verify password before training
def psw():
    assure_path_exists("TrainingImageLabel/")
...
# Capture face images and store them for training
def TakeImages():
    def capture_images():
        try:
...
# Extract face images and corresponding IDs
...
try:
    ...
    message3 = tk.Label(window, text="SmartFace Attendence", fg=PRIMARY_COLOR, bg=BG_COLOR, width=55, height=1, font=('Helvetica', 29, 'bold'))
    ...
    date_label = tk.Label(inner_frame,
                      text="Date: " + datetime.datetime.now().strftime("%B %d, %Y"),
                      bg=BG_COLOR,
                      fg="#64748b",
                      font=('Segoe UI', 11))
    ...
    frame1 = tk.Frame(window, bg=CARD_COLOR, bd=0, highlightthickness=0)
    frame1.place(relx=0.08, rely=0.25, relwidth=0.4, relheight=0.7)

    frame2 = tk.Frame(window, bg=CARD_COLOR, bd=0, highlightthickness=0)
    frame2.place(relx=0.52, rely=0.25, relwidth=0.4, relheight=0.7)

    frame1.config(highlightbackground="#d1d5db", highlightthickness=1)
    frame2.config(highlightbackground="#d1d5db", highlightthickness=1)
    ...
    # Treeview to display registered students
